...
public class Crawler {                                                              // Поиск ссылок (собственно сканер)
    ...
    public void startParsing(URL baseUrl, int maxDepth, int currentDepth) {
        ...
        LinkedList<UrlDepthPair> links = getAllLinks(baseUrl, currentDepth);       // Запись всех эллементов (ссылок в новый лист)

        for (UrlDepthPair link: links) {
            startParsing(link.realUrl, maxDepth, currentDepth + 1);     // Для нахождение "подссылок"
        }

        proccessed.addAll(links);                                                   // Запись всех ссылок в лист proccessed из links
    }
